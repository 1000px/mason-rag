...
13. 如果提供了「上一轮查询上下文」，且用户的问题较短/省略了表名列名，请基于上一轮的查询结果来理解用户意图，复用上一轮的表和列
...
class SqlQueryEngine:
    ...
    def can_handle(self, question: str, context: Any | None = None) -> bool:
        if not self.table_store.get_table_names():
            return False

        if context and context.is_likely_followup(question) and context.last_turn():
            return True
    ...
    def query(self, question: str, context: Any | None = None) -> Dict[str, Any]:
        ...
        for attempt in range(self.max_retries + 1):
            try:
                sql = self._generate_sql(schema, question, last_error, context)
    ...
    def _generate_sql(self, schema: str, question: str, last_error: Optional[str] = None, context: Any | None = None) -> str:
        ctx_text = ""
        if context:
            ctx_text = context.context_for_sql()
        prompt = SQL_SYSTEM_PROMPT.format(schema=schema, question=question, context=ctx_text)
        if last_error:
